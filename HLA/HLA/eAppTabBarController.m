    ...
    if (selectedIndex != _selectedIndex && selectedIndex < [self.viewControllers count])
    {
        ...
        if (selectedIndex == 0) {
            ...
            if (selectedIndex == 1 || selectedIndex == 2 ||selectedIndex == 3 ) {
				...
				if (selectedIndex == 2) {
					[[NSNotificationCenter defaultCenter] postNotificationName:@"ReloadPendingVC" object:self];
				}
                
				if (selectedIndex == 3) {
					[[NSNotificationCenter defaultCenter] postNotificationName:@"ReloadSubmittedVC" object:self];
				}
                [self updateTabBar];
            }
        }
