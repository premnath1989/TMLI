    ...
    if([self Validation] == TRUE){
        ...
        if (sqlite3_open(dbpath, &contactDB) == SQLITE_OK)
        {
            ...
            if (sqlite3_prepare_v2(contactDB, query_stmt, -1, &statement, NULL) == SQLITE_OK)
            {
                ...
                {
                    if (FirstTimeLogin == 1) {
                        NSString *FinalSQL = [NSString stringWithFormat:@"UPDATE Agent_Profile SET AgentPassword= \"%@\", FirstLogin = 0 WHERE IndexNo=\"%d\"", self.ChangePwdPassword ,self.indexNo];
		...
		if ([string isEqualToString:@""]) {
			...
			*/
			if([string isEqualToString:@"Account suspended."]){
				...
				{
					if (FirstTimeLogin == 1) {
						NSString *FinalSQL = [NSString stringWithFormat:@"UPDATE Agent_Profile SET FirstLogin = 1, AgentCode = \"\", AgentName = \"\", "
				...
				{
					if (FirstTimeLogin == 1) {
						NSString *FinalSQL = [NSString stringWithFormat:@"UPDATE Agent_Profile SET FirstLogin = 1 WHERE IndexNo=\"%d\"", self.indexNo];
