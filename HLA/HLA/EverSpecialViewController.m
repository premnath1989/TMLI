...
@synthesize outletReduce,outletWithdrawal,getAge, lblReduceAt, lblReduceTo, getBasicSA, requesteProposalStatus, EAPPorSI, outletDone;
@synthesize outletEAPP, outletSpace,getBasicTerm;
...
- (void)viewDidLoad
{
	...
	if (Editable == NO) {
		...
		outletReduce.enabled = FALSE;
		outletWithdrawal.enabled = FALSE;
        
        self.outletReduce.alpha = 0.5;
		self.outletWithdrawal.alpha = 0.5;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
	...
	else if(textField.tag == 6)
	{
		if ([txtReduceAt.text isEqualToString:@""]) {
			lblReduceTo.text = [NSString stringWithFormat:@"Min:%d Max:%.0f", [getBasicSA intValue ] , [getBasicSA doubleValue]];
		}
		else{
			if ([txtReduceAt.text intValue ] > 19) {
				...
				lblReduceTo.text = [NSString stringWithFormat:@"Min:%.0f Max:%.0f", [getBasicSA doubleValue ] * (0.05 * ([txtReduceAt.text intValue ] -3)  + 0.15) , [getBasicSA doubleValue]];
			}
		}
	}
...
-(void)toggle{
 ...
 -(BOOL)Validation{
	 	...
	 	if (outletReduce.selectedSegmentIndex == 1 && outletWithdrawal.selectedSegmentIndex == 1 && WithdrawExist == FALSE && ReduceExist == FALSE)   {
			return TRUE;
		}
	
	 	if (outletWithdrawal.selectedSegmentIndex == 0) {
            if ([txtAmount.text doubleValue] < 500  )
            {
                UIAlertView *failAlert = [[UIAlertView alloc] initWithTitle:@" "
                                                                    message:@"Minimum Regular Withdrawal is 500." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
                [failAlert show];
				...
				if ([txtStartTo.text integerValue] > getBasicTerm){
				...
				if([txtInterval.text intValue] > getBasicTerm  ){
					UIAlertView *failAlert = [[UIAlertView alloc] initWithTitle:@" "
																	message:[NSString stringWithFormat:@"Interval (Years) for Regular Withdrawal must be within (1-%d).", getBasicTerm ] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
					[failAlert show];
					[txtInterval becomeFirstResponder];
					return FALSE;
				}
		 		...
		 		if([txtAmount.text isEqualToString:@""] || [txtAmount.text isEqualToString:@"0"]){
			 			UIAlertView *failAlert = [[UIAlertView alloc] initWithTitle:@" " message:@"Amount for Regular Withdrawal is required." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
			 			[failAlert show];
			 			[txtAmount becomeFirstResponder];
			 			return FALSE;
				}
	 	...
	 	if (outletReduce.selectedSegmentIndex == 0) {
            ...
            if (getAge > 67) {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@" "
                                                                message:@"Age last birthday must be less than or equal to 67 to opt for Reduced Paid Up Option. " delegate:Nil cancelButtonTitle:@"OK" otherButtonTitles:Nil, nil ];
                [alert show];
                return FALSE;
            }
			...
			if ([txtReduceAt.text intValue ] > 19) {
				...
				aa = [getBasicSA doubleValue ] * (0.05 * ([txtReduceAt.text intValue ] -3)  + 0.15);
			}
			 
			
			if ([txtReduceAt.text intValue ] > 2 && [txtReduceTo.text doubleValue ] < floor(aa)) {
			...
			if ([txtReduceAt.text intValue ] <  20 && [txtReduceTo.text doubleValue ] < floor(aa)) {
