...
- (void)displayData {
	...
	results = Nil;
	results = [db executeQuery:@"select * from  eProposal_LA_Details where eProposalNo = ? and PTypeCode = ?", stringID, @"LA2", Nil];
	while ([results next]) {
...
- (void)viewDidUnload {
    ...
    [self setBtnResidenceCountry:nil];
