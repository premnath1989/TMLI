        ...
        if (txtNewPassword.text.length < 6) {
            ...
            if (txtNewPassword.text.length > 20) {
                ...
                if ([txtNewPassword.text isEqualToString:txtConfirmPassword.text]) {
                    ...
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@" " message:@"New Password did not match with the confirmed password." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                    [alert show];
...
- (void) saveData
{
    ...
    if (sqlite3_open(dbpath, &contactDB) == SQLITE_OK)
    {
        ...
        NSString *querySQL = [NSString stringWithFormat:@"UPDATE Agent_Profile SET AgentPassword= \"%@\" ,FirstLogin = 0 WHERE IndexNo=\"%d\"",txtNewPassword.text,self.userID];
