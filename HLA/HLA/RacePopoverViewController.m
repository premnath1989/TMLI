...
@interface RacePopoverViewController ()

@end

@implementation RacePopoverViewController
...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        _IDTypes = [dict objectForKey:@"Race"];//[NSMutableArray array];
...
{
    //if (indexPath.row == 0)
	NSString *selectedRaceType = [_IDTypes objectAtIndex:indexPath.row];
	NSLog(@"selected id :%@",selectedRaceType);
    
    
    //Notify the delegate if it exists.
    if (_delegate != nil) {
        [_delegate selectedRace:selectedRaceType];
    }
