...
- (void)drawRect:(CGRect)rect {
    ...
    for(int index = 0 ; index + _tvSliderView.minimumValue <= _tvSliderView.maximumValue ; index ++){
        ...
        [value drawAtPoint:CGPointMake(markerImageView.frame.origin.x - (size.width/2), markerImageView.frame.origin.y + markerImageView.frame.size.height + _markerValueOffsetFromSlider) withFont:[UIFont boldSystemFontOfSize:16]];
    }
}
