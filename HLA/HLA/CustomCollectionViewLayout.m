...
- (CGSize)sizeForItemWithColumnIndex:(NSUInteger)columnIndex
{
    ...
    NSString *text = @"HLA Ever Green 2035";
    /*
    ...
    ///CGSize size = [text sizeWithAttributes: @{NSFontAttributeName:[UIFont fontWithName:@"TreBuchet MS" size:14]}];
    CGSize size = CGSizeMake(130, 20);
    
    if (columnIndex == 0) {
        size.width += 12; // In our design the first column should be the widest one
    }
    //return CGSizeMake([@(size.width + 9) floatValue], 50); // Extra space of 9px for all the items
    int offsetX=0;
    if (columnIndex == 0) {
        offsetX = 0;
    }
    return CGSizeMake(115+offsetX,50);
}
