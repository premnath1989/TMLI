...
@interface SPAJ_Perusahaan ()

@end

@implementation SPAJ_Perusahaan
...
- (void)savetoDB:(NSDictionary *)params{
    ...
    [modifiedParams setObject:[[modelSPAJHtml selectActiveHtmlForSection:@"PR"] valueForKey:@"SPAJHtmlID"] forKey:@"SPAJHtmlID"];
    ...
    if ([arraySPAJAnswers count]>0){
        for (int i = 0;i<[arraySPAJAnswers count];i++){
            ...
            [tempDict setObject:@"PR" forKey:@"SPAJHtmlSection"];
    ...
    [delegate voidSetPerusahaanBoolValidate:true];
}
...
- (NSMutableDictionary*)readfromDB:(NSMutableDictionary*) params{
    ...
    NSString* stringWhere = [NSString stringWithFormat:@"where CustomerID=%@ and SPAJID=%@ and SPAJTransactionID=%@ and SPAJHtmlSection='PR'",@"1",@"1",SPAJTransactionID];
